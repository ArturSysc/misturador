...
const ItemForm = ({ onSubmit, itemToEdit, onCancel }) => {
    ...
    return (
        <Form onSubmit={handleSubmit} className="p-3" style={{ backgroundColor: '#f8f9fa', borderRadius: '8px' }}>
            <Form.Group controlId="formName" className="mb-3">
                <Form.Label>{t('tipo1Form.name')}</Form.Label>
                <Form.Control
                    type="text"
                    name="name"
                    value={formData.name}
                    ...
                    required
                    placeholder={t('tipo1Form.enterName')}
                />
            </Form.Group>
            ...
            <div className="d-flex justify-content-start">
                <Button
                    ...
                    style={{ color: 'black'}}
                >
                    <FiCheck size={20} color="black" /> {itemToEdit ? t('tipo1Form.update') : t('tipo1Form.create')}
                </Button>
                {itemToEdit && (
                    <Button
                        variant="link"
                    ...
                    >
                        <FiX size={20} color="red" /> {t('tipo1Form.cancel')}
                    </Button>
                )}
